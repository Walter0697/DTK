...
Use `dtk install-dummy` when you want the broader bundled sample set, including the TOML Cargo.lock-style example, a TOML Python manifest example, a CSV inventory export example, an INI plugin registry example, a XAML ResourceDictionary example, the YAML Kubernetes example config, and their sample payloads.
...
When a source needs an explicit parser choice, add an optional `format` field to the config, for example `"format": "yaml"`, `"format": "toml"`, `"format": "csv"`, `"format": "ini"`, or `"format": "xaml"`.
