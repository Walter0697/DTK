...
pub(super) fn install_codex_guidance() -> io::Result<bool> {
    let mut changed = false;
    changed |= install_text_file(codex_dir().join("DTK.md"), DTK_GUIDE)?;
    Ok(changed)
}

pub(super) fn uninstall_codex_guidance() -> io::Result<bool> {
    remove_if_exists(codex_dir().join("DTK.md"))
}

pub(super) fn install_codex_agents_attachment() -> io::Result<bool> {
...
pub(super) fn uninstall_codex_agents_attachment() -> io::Result<bool> {
    let path = codex_dir().join("AGENTS.md");
    let guide_path = codex_dir().join("DTK.md");
...
pub(super) fn install_codex_skill() -> io::Result<bool> {
    install_text_file(
